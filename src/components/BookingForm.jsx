    ...
    <section id="booking" ref={formRef} className="bg-bg py-16 md:py-24 px-4 md:px-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          ...
          {/* Form */}
          <div className="lg:col-span-3 card p-6 shadow-lg shadow-black/20">
            {submitted ? (
              <div className="flex flex-col items-center justify-center py-12 text-center gap-5 scale-in">
                <div className="w-16 h-16 rounded-full bg-gold/15 border border-gold/40 flex items-center justify-center shadow-[0_0_30px_rgba(212,175,55,0.2)]">
                  <svg width="28" height="28" viewBox="0 0 28 28" fill="none" stroke="#D4AF37" strokeWidth="2.5">
                  ...
                  {STEPS.map((step, i) => (
                    <div key={step.id} className="flex items-start gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center shrink-0 transition-colors duration-500 ${
                          step.done ? 'bg-gold border-gold' : 'border-white/15'
                        }`}>
                          {step.done && (
                        ...
                        {i < STEPS.length - 1 && (
                          <div className={`w-px h-6 mt-1 transition-colors duration-500 ${step.done ? 'bg-gold/40' : 'bg-border'}`}/>
                        )}
                      </div>
                      <div className="pb-4">
              ...
              <form onSubmit={handleSubmit} noValidate className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="btn-gold w-full flex items-center justify-center gap-2 disabled:opacity-60 text-[15px] shadow-[0_4px_20px_rgba(212,175,55,0.2)]"
                >
                  {loading ? (
                    <>
              ...
              <div className="space-y-1">
                {[
                  ...
                  <div key={step} className="flex items-start gap-4">
                    <div className="flex flex-col items-center">
                      <div className={`w-8 h-8 rounded-full border flex items-center justify-center shrink-0 text-[11px] font-extrabold transition-colors duration-300 ${
                        i === 0 ? 'bg-gold border-gold text-bg shadow-[0_0_15px_rgba(212,175,55,0.4)]' : 'border-border text-white/30'
                      }`}>
                        {i === 0 ? (
                          <svg width="12" height="12" viewBox="0 0 10 10" fill="none" stroke="#0F0E0D" strokeWidth="2.5">
                            <polyline points="2,5 4,7 8,3"/>
                          </svg>
                        ) : step}
                      </div>
                      {i < arr.length - 1 && <div className="w-px h-6 bg-border mt-1.5"/>}
                    </div>
                    <div className="pb-4 pt-1">
                      <p className={`text-sm font-bold tracking-tight ${i === 0 ? 'text-gold' : 'text-white/60'}`}>{title}</p>
            ...
            <div className="card p-5 flex flex-col gap-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href="mailto:semtraveluz@mail.ru" className="text-xs text-white/70 mt-0.5 hover:text-gold transition-colors">semtraveluz@mail.ru</a>
                </div>
              </div>
